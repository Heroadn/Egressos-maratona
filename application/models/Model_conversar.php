...
class Model_conversar extends CI_Model
{
    public function fetch($id_grupo = 0,$limit = 10, $start = 0){
        ...
        $data = $this->Model_mensagem->fetch_data($limit, $start, $id_grupo);
    ...
    public function build($id_mensagem){
        $output = '';
        $this->load->model('Model_mensagem');
        $row = $this->Model_mensagem->buscarMensagem($id_mensagem);
    ...
    public function gera_form($action){
        ...
        $posts["input_conteudo"] = form_input(array("name" => "conteudo", "id" => "conteudo", "class" => "", "maxlength" => "2555", "value" => strip_tags(htmlspecialchars_decode(set_value('conteudo'))),"onkeyup" => "handleKeyUp(event);"));
